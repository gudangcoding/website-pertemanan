...
$query_jum_kom = mysql_query("SELECT K.*, U.gambar_profil_kecil FROM komentar K, user U 
                              WHERE K.uid_fk=U.uid AND K.idstatus_fk='$idstatus' 
                              ORDER BY K.idkomentar");
$komentar_count=mysql_num_rows($query_jum_kom);
if($komentar_count>2){
		$koment_seluruhnya=$komentar_count-2;
?>
<div class="commentboxfirst" id='commentboxfirst<?php echo $idstatus;?>'><a href='#' class='commentcoll' id='<?php echo $idstatus;?>' title='Comment'> 
...
<?php 
$newquery=mysql_query("SELECT K.*, U.nama, U.gambar_profil_kecil FROM komentar K, user U 
                       WHERE K.uid_fk=U.uid AND K.idstatus_fk='$idstatus' 
...
$querykoment = mysql_query("SELECT K.*,  U.nama, U.gambar_profil_kecil FROM komentar K, user U WHERE K.uid_fk=U.uid and K.idstatus_fk='$idstatus' order by K.idkomentar desc ") or die(mysql_error());
